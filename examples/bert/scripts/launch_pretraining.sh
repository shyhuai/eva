...
if [[ "$NNODES" -eq 1 ]]; then
    #NCCL_P2P_DISABLE=1 python -W ignore -m torch.distributed.launch --nproc_per_node=$NGPUS \
    NCCL_P2P_DISABLE=1 torchrun --master_addr=localhost --nproc_per_node=$NGPUS \
        run_pretraining.py $KWARGS
else
    NCCL_P2P_DISABLE=1 python -W ignore -m torch.distributed.launch \
